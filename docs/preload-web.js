(function() {
  ...
  async function fetchXHS(url){try{const r=await fetch(PROXY+encodeURIComponent(url));return r.text()}catch(e){return null}}
  async function xhsResolve(url){try{const h=await fetchXHS(url);if(!h)return{success:false,error:'网络错误'};const m=h.match(/nickname["']?\s*:\s*["']([^"']+)["']/)||h.match(/<title>([^<]+)的个人主页/);return{success:true,nickname:m?m[1]:'未知博主',profileUrl:url}}catch(e){return{success:false,error:e.message}}}
  async function xhsFetchNote(nid,cookie){const ck=cookie.includes('web_session=')?cookie:'web_session='+cookie;try{const h=await fetchXHS('https://www.xiaohongshu.com/explore/'+nid);if(!h)return null;const m=h.match(/window\.__INITIAL_STATE__\s*=\s*({[^<]+?})\s*<\/script>/);if(m){const s=JSON.parse(m[1].replace(/undefined/g,'null'));const nm=s?.note?.noteDetailMap;if(nm){const k=Object.keys(nm)[0];const n=nm[k]?.note;if(n)return{title:n.title||'',content:n.desc||'',time:n.time||0}}}const d=h.match(/<meta[^>]+name="description"[^>]+content="([^"]+)"/i);return{title:'',content:d?d[1]:'',time:0}}catch(e){return null}}
  ...
  window.api = {
    addBlogger:async(u)=>{const r=await xhsResolve(u);if(r.success){const id=Date.now();var bs=B();if(!bs||!Array.isArray(bs))bs=[];bs.push({id,nickname:r.nickname,xhs_url:u,tags:'[]',note_count:0,created_at:new Date().toISOString()});save('bloggers',bs)}return r},
